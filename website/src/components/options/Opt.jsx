...
import { TotalPrice, optionPrice } from '../../redux/slices/FencePrice';
...
const Option = () => {
    ...
    let arr = [
        {
            imgUrl:"https://designit.menards.com/media/Fence/selection/post/wood/1112201.jpg",
        ...
        <div className='border h-full lg:w-4/12 w-full border-b-0' >
            ...
            <div className='flex gap-3 p-2'>
            {
                arr.map((ele,ind) => {
                   return (
                    <div key={ind}>
                <div className='border  flex flex-col gap-3 w-full h-full rounded-md shadow-md p-2' onClick={() =>{ 
                    let total_price = ele.price * totalLength;
                    getFencePost(ele.heading,total_price)
                    }} >
                    <img src={ele.imgUrl} className='w-full  bg-cover bg-center' alt="" style={{cursor:"pointer"}} />
                    <h1>{ele.heading}</h1>
                </div>
        ...
        (!isTable) ? <div className='border h-full w-full border-b-0'>
                          ...
                          <div className='flex gap-3 p-2'>
            {
                arr.map((ele,ind) => {
                   return (
                    <div key={ind}>
                <div className='border  flex flex-col gap-3 w-full h-full rounded-md shadow-md p-2 '  onClick={() => getFencePost(ele.heading)} >
                    <img src={ele.imgUrl} className='w-full  bg-cover bg-center' alt=""  style={{cursor:"pointer"}}/>
                    <h1>{ele.heading}</h1>
                </div>
