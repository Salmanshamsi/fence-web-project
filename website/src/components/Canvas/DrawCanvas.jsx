    ...
    if (Math.ceil(length) > 0) {
      ...
      if (displayEditButton) {
        ...
        canvasRef.current.addEventListener('click', (e) => {
          ...
          if (clickX >= buttonX && clickX <= buttonX + 40 && clickY >= buttonY && clickY <= buttonY + 40) {
            // The "Edit" button was clicked
            handleEditButtonClick({ startX: x1, startY: y1, endX: x2, endY: y2 },index);
          }
  ...
  const handleEditButtonClick = (line, index) => {
    ...
    const handleCanvasClick = (e) => {
      ...
      if (
        ...
        // The "Edit" button was clicked
        handleEditButtonClick({ startX: line.startX, startY: line.startY, endX: line.endX, endY: line.endY }, index);
  ...
  const handleMouseMove = (e) => {
    ...
    if (!intersects) {
      ...
      const drawnAway = lines.every((line) => {
        return (
          !doLinesIntersect({ startX, startY, endX: newX, endY: newY }, { startX: line.endX, startY: line.endY, endX: line.startX, endY: line.startY }) &&
          !doLinesIntersect({ startX, startY, endX: newX, endY: newY }, { startX: line.startX, startY: line.startY, endX: line.endX, endY: line.endY })
        );
      });
  ...
  const handleMouseUp = () => {
    ...
    if (length > 0.00) {
      ...
      if (!intersects) {
        ...
        console.log("The new line intersects with an existing line. Not adding it.");
      }
    }
            ...
            <div className='w-full h-full flex flex-col justify-center items-center overflow-y-hidden' >
              ...
              <div className='w-full h-full p-3 flex items-center justify-between bg-green-500 text-white' >
                  ...
                  <button onClick={(e)=>{
                      e.preventDefault();
                      setIsModalOpen(false);
                  }} className='text-2xl' ><i className="fa-regular fa-circle-xmark"></i></button>
              </div>
              
              <div className='flex h-full items-start flex-col justify-start w-full p-2' >
                <h1 className='text-xl font-bold lg:p-5 p-2' >Enter The Length of Fence :</h1>
                  <div className=' h-full w-full flex justify-center lg:gap-20 gap-16' >
                      <div className='flex flex-col items-center justify-center gap-2'>
                      <input type="number" id='nextSide' name='nextSideVal' placeholder='Enter fts' className='border text-center w-20 h-10 lg:w-24 lg:h-12 rounded-full border-gray-400 ' onChange={(e)=>{
                        e.preventDefault();
                        setEnteredFeet(e.target.value);
                      }} />
                      <h2>(1-1000)</h2>
                      <button className='flex items-center justify-center h-12 w-28 bg-green-500 text-white rounded-full'>Next Side</button>
                      </div>
                      <div className='flex flex-col items-center justify-center gap-2'>
                        <input type="number" id='doneSide' name='doneSideVal' placeholder='Enter in' className='border text-center w-20 h-10 lg:w-24 lg:h-12 rounded-full border-gray-400 ' onChange={(e)=>{
                            e.preventDefault();
                            setEnteredInches(e.target.value);
                          }} />
                        <h2>(0-11)</h2>
                        <button onClick={ModalDoneHandler} className='flex items-center justify-center h-12 w-28 bg-green-500 text-white rounded-full'>Done</button>
                      </div>
                  </div>              
              </div>
                                
              <div className='h-full w-full items-center justify-center flex p-2' >
                    <button onClick={handleRemoveButtonClick} className='h-12 w-24 border border-red-500 rounded-full text-red-500' >Remove</button>
              </div>
            </div>
          </Modal>
      )}
